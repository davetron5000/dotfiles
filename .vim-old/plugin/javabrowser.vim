...
function! s:JavaBrowser_Cleanup()
    if has('syntax')
        silent! syntax clear JavaBrowserTitle
    endif
    match none
    ...
    if valid_cache != ''
        ...
        while cmd_output != ''
            ...
            if end > start
                ...
                while stridx(tscope, ".") > 0
                    let tscope = strpart(tscope, stridx(tscope, ".") + 1)
                endwhile
                "call s:JavaBrowser_Warning_Msg('tscope='.tscope)
            else
            ...
            if stridx(s:jbrowser_def_{ftype}_super_tag_types, ttype) != -1
                ...
                if stridx(l:alltypes, l:thistype) == -1
                    ...
                    let l:alltypes = l:alltypes . thistype . "\n"
                endif
    ...
    while i <= s:jbrowser_{ftype}_count
        ...
        if b:jbrowser_{ftype}_{ttype} != ''
            ...
            while l:types != ''
                ...
                endwhile
                let l:tmpalltypes = l:alltypes
                while l:tmpalltypes != ''
                    " Extract one line at a time
                    let l:type_sub_type = strpart(l:tmpalltypes, 0, stridx(l:tmpalltypes, "\n"))
                    " Remove the line
                    let l:tmpalltypes = strpart(l:tmpalltypes, stridx(l:tmpalltypes, "\n") + 1)
                    if exists('l:'.l:type_sub_type) && stridx(l:type_sub_type, l:type) != -1
